var tmpRegs=["[{$hwPun}{$fwPun}w ]".fmtReg(strCommon),"([{$han}{$fwPun}])([0-9a-z])".fmtReg(strCommon,"i"),"([0-9a-z])([{$han}{$fwPun}])".fmtReg(strCommon,"i")];
function doSplit(a,c,d){if(0===a.trim().len()||/^\u3000{2,}/gm.test(a)||a==="\uff0a".times(35))return a+d;var f="",e="\u3000\u3000"+a.trim();a=0;var g=g||2*configs.Linenum;if(g>e.len())return e+d;for(d=Math.floor(e.len()/g)+1;a<d;a++){var b=e.realSubstring(0,g);b=b.replace(/([\u300c\u201c\u300a\u300e\u2018\uff08]){1,2}$/gm,function(a){e+=a;return""});e=e.realSubstring(b.len()).replace(/^([\uff0c\u3002\uff1a\u3001\uff1b\uff1a\uff1f\uff01\uff0e\uff09\u300b\u300d\u300f]{1,2})/gm,function(a){b+=a;return""}).replace(/^([\u2026\uff5e\uff0d])$/gm,
function(a){b+=a;return""});var h=0<b.replace(tmpRegs[0],"").length;if(b.len()<g&&h){var k=tmpRegs[1].test(b)?tmpRegs[1]:tmpRegs[2];b=b.replace(k,"$1 $2").replace(/([\u201c\u2018\u300e\u300c] .*[\u300d\u300f\u2019\u201d])/i,function(a){return a.replace(/([\u201c\u2018\u300e\u300c]) /g,"$1").replace(/ ([\u300d\u300f\u2019\u201d])/g,"$1")+" "});b.len()<g&&h&&(b=b.replace(/ ?([0-9a-z]) ?/i,"$1").replace(/ ([\u201c\u2018\u300e\u300c])/g,"$1").replace(/([\u300d\u300f\u2019\u201d]) /g,"$1"))}f+=b+"\n"}return f+
c}
...
function setTitle(){var a=/(^[\uff08\u3010\u201c\u300c<]|[\uff09\u3011\u201d\u300d>]$)/g,c=$("#inputBookName").val(),d=$("#inputChapter").val(),f="";0<c.length&&(f="\u3010"+c.trim().replace(a,"")+"\u3011");0<d.length&&(f+="\uff08"+d.trim().replace(a,"")+"\uff09");$("#TitleMsg").html(f||$("#TitleMsg").attr("data-tip"))};
