...
function(f){return-1<f.indexOf("\u3002")?f:f.replace("[{$b}]".chapReg("g")," ")}).replace(b.chapReg("gim",g),d)},replaceTitle:function(a,b,g,d){a=a||"\n\n";b=b||"";var f=regChapter,m=configs.regSkipTitle;d&&(f.s=f.sn,f.es=f.e);var l=function(c){return c.replaceAt(configs.sNumber).replace(/\d+/,function(e){e=~~e;return 10>e?"0"+e:e})},n=function(c,e){c=c.replace(/  +/g," ").replace("^[{$sep}]+".chapReg(),"");if(0===c.length)return"";if(-1<c.search(/^\uff08?\d{1,3}\uff09?$/))return c.replace(/^\uff08?(\d{1,3})\uff09?$/,
