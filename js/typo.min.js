...
a.substring(1)}).replace(/\n\n\n+/g,"\n\n").replace(/^\n+/,"").replace(/\n\n+$/,"\n")},__endFixed:function(){var a=configs.rEndFixed;return this.replace(a.Latitude,function(b){return b.replaceAt(a.LatitudeAt)}).replace(a.Number,function(b){b=b.replaceAt(a.NumberAt);/\d+,\d{3}\b/.test(b)||(b=b.replace(/,/g,"\uff0c"));return b}).replace(/(?:\d+,){2,10}\d+\u2026\u2026/g,function(b){return b.replace(/,/g,"\uff0c")}).replace(a.Time,function(b){return b.replaceAt(a.TimeAt).matchUpper(/[apm]/g)})},replaceSpace:function(){return this.cleanSpace(configs.hanSpace)},
replaceSeparator:function(){return this.replaces(configs.rSeparator).replace(/@@{3,}/g,configs.Separator)},replaceQuotes:function(a){var b=this.replace(configs.enSep,"$1\u3014\u203b@\u203b\u3015$2").replaces(configs.rQuotes).replace(/\u3014\u203b@\u203b\u3015/g,"'");return"cn"===a?b.replaceAt(configs.cnQuotes):b},matchUpper:function(a){return this.replace(a,function(b){return b.toUpperCase()})},matchLower:function(a){return this.replace(a,function(b){return b.toLowerCase()})},convertEngLine:function(){var a=
configs.findEng;return this.replace(a.Line,function(b){return b.find(/^[\u201c\u300c][a-z]{1,6}(?:\u3002|[\uff01\uff1f]{1,3}|\u2026\u2026|\u2014\u2014)[\u300d\u201d]$/mi)||2>b.findCount(a.LineSkip)||!b.find(/[,\uff0c ]/)||b.find(/[\u300a\u300b]/)?b:b.replaceAt(configs.halfPunsOther).replaceAt(configs.halfPuns).replaces(configs.halfSymbol).matchLower(a.PunAfter).matchUpper(/[\.\?!:&] ?[a-z]/g).trim()})},convertEnglish:function(){var a=configs.findEng,b=a.Word,h;return this.matchLower(/[A-Z]/g).matchUpper(/\b[a-z]/g).replace(a.Sep,
function(c){return c.replace("[{$enSep}]+".comReg("g"),"'")}).replace(a.NameSep,function(c){return c.replace("[{$enSep}]+".comReg("g"),"'")}).replace(a.Bracket,function(c){return-1<c.indexOf(" ")?c:c.toUpperCase()}).replace(a.PunFix,function(c){return-1<c.indexOf("'")?c.matchLower(/ [A-Z]/g):c}).replace(/\b[\w\-~\uff5e]+\b/g,function(c){return c.find(/\d/)?c.toUpperCase():c}).replace(a.HonorWord,function(c){return c.matchUpper(/\b[a-z]/g).replace(/\u3002/g,".")}).matchUpper(("\\b(?:"+a.WordUpper+
")\\b").getReg("gi")).replace(("\\b("+b+")([0-9]{1,4}[a-z]{0,6})?\\b").getReg("gi"),function(c,e,m){h=("|"+b.toLowerCase()+"|").indexOf("|"+e.toLowerCase()+"|")+1;return("|"+b+"|").substr(h,e.length)+m.toUpperCase()}).matchUpper(a.Continuous).replace(a.LowerExt,function(c){return c.toLowerCase().replace(/\u3002/g,".")}).replace(a.Site,function(c){return c.toLowerCase().replaceAt(a.SiteFix)}).replaces(a.WordFix).matchUpper(/\b[a-z]\b/g).matchLower(("\\b(?:"+a.WordLower+")\\b").getReg("gi")).replace(a.NameFix,
function(c){return c.slice(0,c.length-1)+c.slice(-1).toUpperCase()}).matchLower(/'\b[a-z]+ /gi).replace(/^[a-z]+ /gim,function(c){return c.matchUpper(/\b[a-z]/gi)})},convertNumberLetter:function(a){return this.replace(/([\uff21-\uff3a][\uff41-\uff5a]+)(?=[\uff21-\uff3a][\uff41-\uff5a]+)/g,"$1 ").replaceAt(configs.sNumberLetter)},__chapterFullNumber:function(){return this.replace(configs.regFullNumberTitle,function(a){return a.replaceAt(configs.sNumber)})},convertFullNumberLetter:function(){return this.replaceAt(configs.sNumberLetter,
!0).__chapterFullNumber()},convertUnicode:function(){return this.replace(/(?:[\uff06&]#x|\\u?)[\da-f]{4}[;\uff1b]?/gi,function(a){a=a.replace(/[\uff06&]#x|\\u|[;\uff1b]/g,"");return unescape("%u"+a)}).replace(/\\x[\da-f]{2}/gi,function(a){a=a.replace(/\\x/,"");return unescape("%u00"+a)}).replace(/[\uff06&]#\d+[;\uff1b]/g,function(a){a=a.replace(/[^\d]/g,"");return String.fromCharCode(a)}).replace(/(?:[%\uff05][\da-f]{2})+/gi,function(a){try{return decodeURIComponent(a.replace(/\uff05/g,"%"))}catch(b){return a}})},
convertHtmlEntity:function(){return this.replace(configs.regHtmlEntity,function(a){var b=a.replace(/\W/g,"");return configs.sHtmlEntity[b]||configs.sHtmlEntity[b.toLowerCase()]||a})},convertVariant:function(){return this.replaceAt(configs.sVariants)},convertSerialNumber:function(){var a=configs.sSerialNumber,b=a[1].split("|");return this.replace(("["+a[0]+"]").getReg(),function(h){return"\uff08"+(b[a[0].indexOf(h)]||"")+"\uff09"})},convertPunctuation:function(){return this.replaceAt(configs.punSymbol).replaces(configs.punSymbolFix).replaces(configs.nwSymbol)},
