...
Object.extend(String.prototype, {
	...
	convertInitial: function() {
		...
		for (var i = 0; i < pWord.length; i++)
			re = re.replace(('\\b(' + pWord[i] + ')([0-9]*)\\b').getReg('gi'), pWord[i] + '$2')
		return re
	},
	// 全角半角字母数字，ve=1时全角
	...
	replaceSpace: function() {
		return this.space()
			...
			// 半角标点 \/\\\+\-\'
			// 半角标点 \@\&\=\_\,\.\?\!\$\%\^\*\-\+\/\(\)\[\]\'\"<\>\\
			.replace('[ ]+(?=[{$han}{$fwPun}])'.fmtReg(regCommon), '')
			// 英文数字后跟全角标点
			.replace('([0-9a-z])[ ]+([{$han}{$fwPun}])'.fmtReg(regCommon, 'gi'), '$1$2')
			.replace('([{$han}{$fwPun}])[ ]+([0-9a-z])'.fmtReg(regCommon, 'gi'), '$1$2')
		// 英文数字后跟汉字
	...
	replaceQuotes: function(d, simp) {
		...
		var re = this.replace('([a-z])([{$enSep}])([a-z])'.fmtReg(regCommon, 'gi'), '$1※@※$3')
		if (!simp) {
			re = re.replace(/[‘’『』]/g, '\'')
				.replace(/[“”「」]/g, '\"')
		}
