...
var regChapter = {
	...
	/****** 非常规标题·可有后续主体 ******/
	't1': [
		'楔子|引[言子]|序篇?章|序[言幕目曲传傳卷]?|导读|導讀|[后後][记话記話序]|尾[声记聲記]|[附目][录錄][0-9]{0,2}|正文',
...
// 处理自由组合的标题
String.prototype.chapReg = function(f, r) {
	return this.fmt(regChapter).fmtReg(strChapter, f || 'gm', r)
}
String.prototype.comReg = function(f) {
	return this.fmtReg(strCommon, f || 'gm')
}
...
var configs = {
	...
	'regSkipTitle': {
		...
		't2': /^第?[零一二三四五六七八九十百两](?:部[分]|季[度]|篇[篇经文]|[部集](?:戏|戲|电[影视視]|的)|部好莱坞|回合)|^一(?:幕[幕]|回[生首头家頭閤]|[直切生世])|^二(?:话[没不沒])|^三(?:[生世])/m,
		't3': '^[一二三四五六七八九十百两兩][{$c}].*[，].*$|^{$n2}[年月日]'.fmtReg(strChapter, 'm'),
		't6': [
	...
	'enSepF': '([a-z])[{$enSep}]'.comReg('gi'),
	'enSepE': '[{$enSep}](?=[ a-z])'.comReg('gi'),
	'enSep': '([a-z])[{$enSep}]|[{$enSep}]([ a-z])'.comReg('gi'),
	// 英文间单引号样式替换
	'enSepQ': '\\b([a-z]+)[{$enSep}]( |[a-z]+\\b)'.comReg('gi'),
	// 汉字间空格
	'hanSpace': ' +(?=[{$han}{$fwPun}{$qfwPun}])'.comReg(),
	// 英文间空格
	'engSpace': '([{$han}{$fwPun}{$qfwPun}]) +(?=[0-9a-zA-Z])'.comReg(),
	// 英文单独一行
	'findEngLine': '^[\\w {$hwPun}{$qhwPun}{$fwPun}{$qfwPun}]{2,}$'.comReg('gm'),
	'findEngLineSkip': '[\\d {$hwPun}{$qhwPun}{$fwPun}{$qfwPun}]'.comReg(),
	// 引用中的英文
	'findEngQuote': '[（〈《【〔〖“‘「『]([\\w {$hwPun}{$qhwPun}{$fwPun}]{2,})[』」’”〗〕】》〉）]'.comReg('gm'),
	'findEngQuoteTest': /[，。！？…]/,
	// 称谓单词前缀
	...
	rEnd: [
		...
		[/ ?co[。\.][，\,]? ?ltd[。\.]?/gi, ' Co.,Ltd.'],
		// 处理特殊情况
		[/[。\.](avi|jpg|bmp|png|com|net|org|gov|edu|uk|ru|cn|jp)\b/gi, function(m, m1) {
