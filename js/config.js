...
var configs = {
	...
	/****** 标题正则设定 ******/
	// 标题行首空格
	regStart: '(?:[{$s}]*)'.fmt(Space.regSafe()),
	// 标题间隔符（严格限定）
	regSeparator: '[{$s}\\.\\,\\:\\-。，：—、]{1,6}'.fmt(Space.regSafe()),
	// 标题间隔符
	regSeparatorNull: '[{$s}\\.\\,\\:\\-。，：—、]{0,6}'.fmt(Space.regSafe()),
	// 行尾
	regEnd: '.{0,30}[^。：;；”」\\n]',
	// 行尾（严格限定）
	regStrictEnd: '.{0,30}[^，。：;；、…？！\\?\\!“”「」\\n]',
	...
	regTitle: {
		/****** 非常规标题 ******/
		t1: [
			'[引楔]子|引言|序篇?章|自?序[言幕]?|题[注记]|題[註記]|文案|卷[首后後][语語]|开卷[语語]|(?:作者)?前言|[全本下][{$c.0}{$c.1}](?:[简簡]介|介[绍紹]|[预預]告)|(?:作品|作者|人物|内容)?(?:[简簡]介|介[绍紹]|[预預]告|自介)|篇[后後]|[后後][记话記話]|尾[声记聲記]|(?:完本|作者)感言|附[录錄]|作者的[话話]|正文'.fmt(regChapterCut),
			'{$f}[{$c.0}{$c.1}]'.fmt(regChapterCut),
			'{$f}[{$c.2}]'.fmt(regChapterCut),
			'同人|[前后外续後續][番传篇傳章]|外{$n.0}{1,5}[章回折节话節話]|[续續]{$n.0}{1,5}|[番篇]外[篇卷]?{$n.0}{0,4}'.fmt(regChapterCut)
		],
		/****** 一章/第一章/一章：标题/第一章：标题 ******/
		t2: '((?:第)?[ ]*{$n.2}{1,9}[ ]*[{$c}])'.fmt(regChapterCut),
		/****** 卷一/卷一：标题 ******/
		t3: '({$cf})[ ]*({$n.1}{1,9})'.fmt(regChapterCut),
		/****** 01/01./01.标题/一/一、标题 ******/
		t4: '({$n.1}{1,9})'.fmt(regChapterCut),
		/****** （一）/（一）标题 ******/
		t5: '([\\(（]{$n.2}{1,9}[\\)）])'.fmt(regChapterCut),
		/****** chapter 22 ******/
		t6: '(?:chapter|☆|★|○|●|◎|◇|◆|□|■|△|▲|※|＃)[ \.]*(\\d+)'
	},
	// 全角数字标题
	...
	punSymbol: [
		...
		[/([“「])[？]{1,}[！]{0,}([」”])/g, '$1？！！$2'],
		// 行尾注释
		[/([（][^（）\d０-９一二三四五六七八九十百千万]+[）])$/gm, '\n$1'],
		[/^([（][^（）\d０-９一二三四五六七八九十百千万]+[）])/gm, '$1\n'],
		[/\n+([（][^（）\d０-９一二三四五六七八九十百千万]+[）])\n+/gm, '\n$1\n']
	],
	// HTML 字符实体
