...
var _rcc = configs.regChapterCut
// 标题正则
configs.regTitle = {
	/****** 非常规标题 ******/
	t1: [
		'[引楔]子|引言|序[篇]?章|[自]?序[言幕]?|题[注记]|題[註記]|文案|卷[首后後][语語]|开卷[语語]|(?:作者)?前言|[全本下][≮c.0≯≮c.1≯](?:[简簡]介|介[绍紹]|[预預]告)|(?:作品|作者|人物|内容)?(?:[简簡]介|介[绍紹]|[预預]告|自介)|篇[后後]|[后後][记话記話]|尾[声记聲記]|(?:完本|作者)感言|附[录錄]|作者的[话話]|正文'.fmt(_rcc),
		'≮f≯[≮c.0≯≮c.1≯]'.fmt(_rcc),
		'≮f≯[≮c.2≯]'.fmt(_rcc),
		'同人|[前后外续後續][番传篇傳章]|外≮n.0≯{1,5}[章回折节话節話]|[续續]≮n.0≯{1,5}|[番篇]外[篇卷]?≮n.0≯{0,4}'.fmt(_rcc)
	],
	/****** 一章/第一章/一章：标题/第一章：标题 ******/
	t2: '((?:第)?[ ]*≮n.2≯{1,9}[ ]*[≮c≯])'.fmt(_rcc),
	/****** 卷一/卷一：标题 ******/
	t3: '(≮cf≯)[ ]*(≮n.1≯{1,9})'.fmt(_rcc),
	/****** 01/01./01.标题/一/一、标题 ******/
	t4: '(≮n.1≯{1,9})'.fmt(_rcc),
	/****** （一）/（一）标题 ******/
	t5: '([\\(（]≮n.2≯{1,9}[\\)）])'.fmt(_rcc),
