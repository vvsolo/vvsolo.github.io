...
var configs = {
	...
	regTitle: __fmts({
		...
		't1': [
			'楔子|引[言子]|序篇?章|序[言幕目曲传傳卷]?|[后後][记话記話序]|尾[声记聲記]|[附目][录錄]|正文',
			'{$tf}[{$c.0}{$c.1}]',
			'{$tf}[{$c.2}]',
			'同人|[前后外续後續间間][番传篇傳章]|外{$n.0}{1,5}[章回折节话節話]|[续續]{$n.0}{1,5}|[番篇]外[篇卷]?{$n.0}{0,4}'
		],
		/****** 一章/第一章/一章：标题/第一章：标题 ******/
		't2': '((?:第)?[ ]*{$n.2}{1,9}[ ]*[{$c}])',
		/****** 卷一/卷一：标题 ******/
	...
	regSkipTitle: {
		't0': /([。…—！？][」”]|。)$/,
		't1': /^(序[长词战兴常稿歌秩次传述長詞戰興傳]|[上下]回|[\-—]{1,4})/,
		't2': /^([第]?[0-9０-９零一二三四五六七八九十两]{1,9})(部[分]|季[度]|篇[篇文]|幕[幕]|话[没不沒]|回[生首头合家頭閤]|[部集](戏|戲|电[影视視])|部好莱坞)/,
		't4': [
	...
	nwSymbol: [
		...
		// 2018年9月6日4:11PM
		[/(\d{2,4}[年\-\—]\d{1,2}[月\-\—]\d{1,2}[日\-\—])[ ]*(\d{1,2}\:\d{1,2})[ ]*(AM|PM)/gi, '$1 $2$3'],
		[/(\d{2,4}[年\-\—]\d{1,2}[月\-\—]\d{1,2}[日\-\—])[ ]*(\d{1,2}\:\d{1,2})(AM|PM)([^。\n])/gi, '$1 $2$3\n$4'],
		// 英文连接符–，暂弃用
		[/([^-—～─])[-—～─]([\d\.a-z]+)/gi, '$1-$2'],
		[/([\d\.a-z]+)[-—～─]([^-—～─])/gi, '$1-$2'],
