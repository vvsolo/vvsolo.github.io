...
/**
 * find     - RegExp 正则查询
 * skip     - RegExp 正则过滤，和 find 配套，不通过
 * pass     - RegExp 正则过滤，和 find 配套，通过
 * at       - Array  字符逐字替换
 * rp       - Array  字符替换
 * rps      - Array  字符批量替换
 * mu       - RegExp 正则查询 大写字母
...
config.rEng = {
	// 所有字母
	'Alphabets': '[a-zA-Z{$latin}]+'.comReg('g'),
	// 约定英语，用|分隔
...
config.rEng.Char = [
	...
	{
		'find': [
			'\\b[a-zA-Z]+[{$enSep}](?: |\\b[a-zA-Z]+\\b)'.comReg('g'),
			'\\b[iodlIODL][{$enSep}]'.comReg('g')
		],
		'rp': config.enSepTo
	},
	// 英文连接符 –
	...
	// 常用习惯大写
	{ 'mu': '(?![{$latin}])\\b(?:{$zz})\\b'.fmtReg(config.rEng.Upper, 'gi') }
]
/***** 英文结尾 *****/
config.rEng.End = [
	// 姓名缩写后大写
	...
	// 称谓单词前缀
	{
		'find': /\b(?:Mrs?|Ms|Doc|Dr|Jr|Rev|Hon|Mmes?|Esq|Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sept|Oct|Nov|Dec)\b。(?!$)/gi,
		'lc': 'f',
		'rp': [/。/g, '.']
	},
	// 特殊的连续单词 转大写
	{ 'mu': /\b(?:([a-z])\1+|abcdefg|abcdef|abcde|abcd|abc|xyz|jqka?|wsad|wasd|qwerty)\b/gi },
	// 小写的后缀
	{
		'find': /。\b(?:Txt|Avi|Jpe?g|Bmp|Png|Net|Gif|Ppt|Webp)\b/g,
		'lc': 'l',
		'rp': [/。/g, '.']
	},
	// 虚词小写，非行首 up
	{ 'ml': '(?!^|[{$latin}])\\b(?:a|about|an|and|as|at|be|but|by|for|from|in|into|is|nor|of|off|on|onto|or|out|over|should|so|the|to|under|will|with|yet|vs)\\b(?![{$latin}])'.comReg('g') },
	// 处理 url
	{
	...
	{ 'mu': '\\b(?:i|ok|n|e|w|s)\\b(?=[^ {$latin}])'.comReg('gi') },
	// 缩拼字母的省市县
	{ 'mu': /\b[a-zA-Z]{1,4}[国省市县山城江河]/g },
	// X 表示乘号时
	{ 'ml': /\b\d+X\d+\b/g },
	// 顶头字母大写 935
	{ 'mf': /(?:^|[“「《〈〔〖（【])\b[a-zA-Z]+ /gm }
...
/****** 结尾修正 ******/
// 临时定义
strCommon['_f0'] = '[0-9]{1,3}(?:[.。][0-9]{1,2})? ?[\x22\x27°{$qfwPun}]'.fmt(strCommon);
...
config.rEndFix = [
	...
	// 经纬度 20"65'
	{
		'find': '\\b{$_f0} *{$_f0}'.comReg('g'),
		'at': ['“‘”’「『」』 ', '\x22\x27\x22\x27\x22\x27\x22\x27']
	},
	// 修正数字间的全角 （）() 21.05.24 去除 ，,
	// 时间 00:00:05，150
	{
		'find': '\\b[a-zA-Z]*{$_f1}(?:[。·.：〉〈—＋－＝＞＜]\\b{$_f1})+'.comReg('g'),
		'skip': ['[{$ofwPun}][.,]'.comReg('')],
		'at': ['。·.：〉〈＊，—＋－＝＞＜', '...:><*,-+-=><']
	},
	// 计量单位后跟标点，还原
	...
	// 21.07.24 修正数字超长时正则引起的锁死
	{
		'find': [
			// 修正数字后缀 20,000.00 12.99%
			'\\b(?:\\d{1,18}|[\\d,，]{2,20})(?:[。.]\\d{1,8})?(?:[a-zA-Z]+\\b|(?:人民币|软妹币?|[韩美日港澳]元|英镑|港币|新?台币|法郎|比索|[金人千万亿份倍帧斤顷个只条包点种座克寸件瓦支次股丈吨厘毫亩元{$ofwPun}])|$)'.comReg('gm'),
			// IP 格式 192.168.0.1
			/\b[12]?\d{1,2}(?:[。.][12]?\d{1,2}){3}\b/g
		],
		'at': ['，。', ',.']
	},
	// 修复字母数字最后的标点
