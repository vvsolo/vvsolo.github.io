...
var getChapters = {
	// 非常规标题0
	't0': { 'tpl': '^({$t0}){$sn}$', 'level': 3 },
	// 非常规标题1
	't1': { 'tpl': '^({$t1})(之{$e}|{$s}{$e}|{$sn}$)$', 'join': '|' },
	// 卷部 集篇阙季册闕冊 章折回话节幕節話
	't2': { 'tpl': '^{$t2}({$sn})({$e}|$)$' },
	// （一）/（一）标题
	't3': { 'tpl': '^{$t3}({$e}|$)$', 'level': 4 },
	// 卷一/卷一：标题
	't4': { 'tpl': '^{$t4}({$s}|{$s}{$e}|$)$', 'level': 2 },
	// chapter 22: 标题
	't5': { 'tpl': '^{$t5}{$sn}({$e}|$)$', 'type': 'relaxe', 'level': 3 },
	// 01/01./01.标题/一/一、/一、标题
	't6': { 'tpl': '^{$t6}({$s}|{$s}{$e}|$)$', 'type': 'relaxe', 'level': 2 }
}

// 处理自由组合的标题
Object.assign(String.prototype, {
	chapFlag: function(f, r) {
		...
		return {
			'str': d.fmt(strChapter),
			'flag': f,
			't': f.replace('g', '')
		}
	},
	chapReg: function(f, r) {
		var d = this.chapFlag(f, r)
		return d.str.getReg(d.flag);
	},
	comReg: function(f) {
		return this.fmtReg(strCommon, (f === '') ? '' : f || 'g');
	}
})
...
var config = {
	...
	/****** 标题忽略 - 正则设定 ******/
	// 全角数字标题
	'regFullNumberTitle': '[第\\(（]{$n2}[{$c}\\)）]'.chapReg('g'),
	// 有序列表的章节标题保护
	'regListTitle': [
		'^(?:{$w1}{$s}.*\\n{1,2}){2,}'.chapReg(),
		'^(?:{$w2}{$s}.*\\n{1,2}){2,}'.chapReg(),
	...
	'regSkipTitle': {
		...
		't2': [
			/^上节[目课]/,
			/^第?[零一二三四五六七八九十百两]{1,3}(?:部分|季度|卷[书经]|篇[篇经文]|[部集](?:戏|戲|电[影视視]|的)|部好莱坞|回合|节课|[爷奶爹娘爸妈舅叔姨婶嫂哥姐弟妹儿女孙侄子季])/,
			'^一(?:[{$c}]|[直切生世味]|回[复合首头到])'.chapReg(''),
			/^(?:二话[没不沒]|三[生世]|四[周边处海]|五[谷更]|[百千][折转])/
		],
		't3': [
			'^(?:{$n1}|{$n2})[年月日]'.chapReg(''),
			/一[，、]二[，、]三/,
	...
	// 汉字间空格
	'hanSpace': ' +[{$han}{$fwPun}{$qfwPun}{$ofwPun}\\\/]|[{$han}{$fwPun}{$qfwPun}{$ofwPun}\\\/] +'.comReg(),
	// 其他符号间空格
	//'charSpace': /, \d/g,
	// 英文间隔
	'enSep': '\\b([a-zA-Z]+)[{$enSep}]( |\\b[a-zA-Z]+\\b)'.comReg(),
	// 英文间隔正则替换
	'enSepTo': ['[{$enSep}]+'.comReg(), "'"],
	// 全半角数字
...
/**
 * hit     - RegExp 正则查询
 * skip     - RegExp 正则过滤，和 hit 配套，不通过
 * pass     - RegExp 正则过滤，和 hit 配套，通过
 * at       - Array  字符逐字替换
 * rp       - Array  字符替换
 * rps      - Array  字符批量替换
 * mu       - RegExp 正则查询 大写字母
 ...
 * ms       - RegExp 正则查询 首字母大写
 * lc       - String 字母大小写，u -> 大写; l -> 小写; f -> 首字母大写
 **/
/***** 英文的批处理 - 全角字符 *****/
// 临时定义
strCommon['_ac'] = '0-9a-zA-Z{$hwPun}{$qhwPun}{$fwPun}{$latin}'.fmt(strCommon);
config.rEnglish = {
// 所有字母
'Alphabets': '[a-zA-Z{$latin}]+'.comReg(),
// 约定英语，用|分隔
...
// 重叠连续单词
'Overlap': /\b(yes|no|go|up|hia|hi|ha|biu|bye|ok)\1+\b/gi,
// 前置的符号处理
'FixChar': [
	// 整句转小写
	{ 'ml': /[a-z]，\b[A-Z]/g },
...
'FixCase': [
	...
	// 称谓单词前缀
	{
		'hit': /\b(?:Mrs?|Ms|Doc|Dr|Jr|Rev|Hon|Mmes?|Esq|Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sept|Oct|Nov|Dec)\b。(?!$)/gi,
		'lc': 'f',
		'rp': [/。/g, '.']
	},
	// 特殊的连续单词 转大写
	{ 'mu': /\b(?:([a-z])\1+|abcd?e?f?g?|xyz|jqka?|wsad|wasd|qwerty)\b/gi },
	// 虚词小写，非行首 up
	{
		'hit': /.\b(?:a|about|an|and|as|at|be|but|by|for|from|in|into|is|nor|of|off|on|onto|or|out|over|should|so|the|to|under|will|with|yet|vs)\b./gi,
		'skip': '^[^{$latin}]|[^{$latin}]$'.comReg(''),
		'lc': 'l'
	},
	// 小写的后缀
	{
		'hit': /。\b(?:Txt|Avi|Jpe?g|Bmp|Png|Net|Gif|Ppt|Webp)\b/g,
		'lc': 'l',
		'rp': [/。/g, '.']
	},
	// 处理 url
	...
	// 处理 email
	{
		'hit': /\b[\w-—]+@\b[\w-—]+\b(?:[。\.]\b[\w-—]+\b)+\b/gi,
		'at': ['。—', '.-'],
		'lc': 'l'
	},
	{
		'hit': /\b(?:i|ok|n|e|w|s)\b./gi,
		'skip': '[ {$latin}]$'.comReg(''),
		'lc': 'u'
	},
	// 缩拼字母的省市县
	...
	// 英文连词 221005
	//{ 'ml': /\b[a-z]+\-[A-Z][a-zA-Z]+\b/g },
	//{ 'mf': /\b[A-Z][a-z]+\-[a-z]+\b/g },
	// 顶头字母大写 935
	{ 'mf': /^\b[a-zA-Z']+ /gm },
	{ 'mf': /[“「《〈〔]\b[a-z]+ /g }
]
...
config.rEndFix = [
	...
	// 数字带中文间的破折号 500两——1000两
	{
		'hit': /\b\d+\b(\W{1,3})[\-—]{1,4}\b\d+\b\1/g,
		'rp': [/—+/g, '-']
	},
	// 经纬度 20"65'
	{
		'hit': '{$_f0} *{$_f0}'.comReg(),
		'at': ['“‘”『』‘’「 」', '\x22\x22\x22\x22\x27\x27\x27\x27']
	},
	// 修正数字间的全角 （）() 21.05.24 去除 ，,
	// 时间 00:00:05，150
	{
		'hit': '\\b[a-zA-Z]*{$_f1}(?:[。·.：〉〈—＋－＝＞＜]\\b{$_f1})+'.comReg(),
		'skip': '[{$ofwPun}][.,]'.comReg(''),
		'at': ['。·.：〉〈＊，—＋－＝＞＜', '...:><*,-+-=><']
	},
	// 计量单位后跟标点，还原
	...
	// (?:\\d{1,18}|[\\d,，]{2,20})(?:[。.]\\d{1,8})?
	{
		//'hit': '\\b(?:\\d{1,18}|\\d{1,3}(?:[,，]\\d{1,3}){1,3})(?:[。.]\\d{1,8})?(?:[a-zA-Z]+\\b|(?:人民币|软妹币?|[韩美日港澳]元|英镑|港币|新?台币|法郎|比索|[千万亿倍斤顷点克寸瓦丈吨亩{$ofwPun}]|[金人份帧个只条包种座件支次股厘毫元]|公[斤里顷])|$)'.comReg(),
		'hits': [
			// 小数
			/\b\d{1,18}[。.]\d{1,8}\b/g,
			// 格式化数字
			/\b\d{1,3}(?:[,，]\d{1,3}){1,3}\b/g,
			// 格式化数字，有小数
			/\b\d{1,3}(?:[,，]\d{1,3}){1,3}[。.]\d{1,8}\b/g
		],
		'at': ['，。', ',.']
	},
	// 小数字点后是三位，还原
	...
	{
		'hits': [
			'\\b\\d+[。.]\\d{3,}\\b(?:人民币|软妹币?|[韩美日港澳]元|英镑|港币|新?台币|法郎|比索|[千万亿倍斤顷点克寸瓦丈吨亩秒{$ofwPun}]|公[斤里顷]|小时|分钟)'.comReg(),
			// IP v4 格式 1.0.0.0 255.255.255.255
			/\b(?:[1-9]|[1-9][0-9]|1[0-9]{2}|25[0-5]|2[0-4][0-9])(?:[。.](?:[0-9]|[1-9][0-9]|1[0-9]{2}|25[0-5]|2[0-4][0-9])){3}\b/g
		],
		'rp': [/。/g, '.']
	},
	// 修复字母数字最后的标点
