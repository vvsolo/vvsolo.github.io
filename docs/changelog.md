...
### 230801 更新日志
- 优化：【目录结构】排版核心功能和【简繁互换】、【随机取名】分开放置
- 新增：【工具栏】添加 `标点检查` 和 `其他检查` 按钮，细分各项可单独操作
- 新增：【快捷键】运行检查后，`Alt+,` 和 `Alt+.` 快捷键，可定位上、下一个提示
- 优化：【章节标题】新增大量章节和非常规标题识别，主章节包括 `卷部集篇册辑季章折回话节幕段阙`
- 优化：【章节修正错误】处理卷章、章节相连，去重；清除章节标题前不必要的 `正文`、`大结局` 等
- 优化：【引号嵌套】引号未成对的段落，尝试修正可以确定的错误引号
- 优化：【04.node】优化代码，确保大文件处理时能返回正确内容
- 新增：【段落重组分隔】将错乱的段落合并一行后，再进行智能分隔
- 新增：【05.correct〔实验性〕】使用 OpenCC 勘误词组、修正拼音和星号，因软件排序和分词原因，可能有错误
- 新增：【09.自动排版】增加 `11.杂项修正`，修正 `引号嵌套`、`行尾缺失`、`清除广告`、`清除多余分隔符`
- 新增：【10.辅助功能】一些有用但不常用的脚本，具体介绍请查看 `在线文档`
- 新增：【校对】增加 `分项检查`，及 `标点检查`、`其他检查` 两个汇总检查
- 优化：【章节检查】优化代码，修正错误
