...
- 更新：OpenCC 版本更新为 1.1.2
- 新增：【大纲视图】新增【小说脚注】，显示 `行内标注` 和 `脚注`（参考：《碧海墨锋》）
- 修正：【简繁转换】自定义中 `澹淡` 的处理
- 新增：设定 F5 为【补齐双引号缺失】
- 新增：【补齐双引号缺失】（未选中文字状态），自动搜索缺失的双引号——前置在冒号后补齐双引号，若无冒号，在段首补齐；后置在段末补齐
- 新增：全局、普通整理和修正错误标题，将自动删除连续相似重复的章节标题
- 修正：【全文检查】过滤单双号混用的重复显示
- 修正：【全文检查】连续多行的错误，合并缩略显示，并在边栏提示图标中加以区别
- 修正：【大纲视图】有 `章` 和 `节` 同时并存的情况下，将后者降级（参考：幻想《烈火凤凰》）
- 修正：【章节标题】大写转数字时的错误
- 修正：【章节标题】数字转大写时的错误
- 修正：【章节标题】修正有序列表——连续省略号方式错误转换（参考：qinqiyan《代女而嫁的蜜月风波》）
- 修正：【章节标题】章节后跟随序号标题（上中下终续/数字序号）时，自动添加括号
- 优化：英文、数字间标点符号修正
- 修正：回车事件触发自动刷新大纲时光标定位错误
