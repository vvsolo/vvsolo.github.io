...
var TextileHighlightRules = function() {
    this.$rules = {
        "start" : [
            {
                token : ["text", "title0"],
                regex : /^([ 　\s]*)((?:序篇章|首部曲|[作译譯编編著]者序|[续續外][0-9０-９零一二三四五六七八九十]?[章折回话节幕段阙節話闕]|(?:楔子|弁言|前[言记記]|引[言子文]|[首小节自次总代節總简原]序|序[章言目曲传傳]|序|[后後附][记话記話序语語]|尾[声记聲記]|大?[结結]局|附[录錄]|同人[续續]?|[前后外续正後續间間转轉][传篇傳章]|完[结結]篇|特[别別]篇|[前后外里裏]番|[番篇]外[篇卷章语語]?)[（\(]?[0-9０-９一二三四五六七八九十上中下续终續終]{0,3}[）\)]?)[： 　，、．·。—.,:|-]{0,4}(?:.{0,40}[^。：;；\n]|$))$/
            }, {
                token : ["text", "title2"],
                regex : /^([ 　\s]*)((?:第?[0-9０-９]{1,5}(?:[-—.][0-9０-９]{1,5})?|第?[0０○〇零一二三四五六七八九十百千壹贰叁肆伍陆柒捌玖拾佰仟两廿卅卌貳叄陸兩]{1,7}|最[初终尾終]|[上下]半|[序初前始后终首末尾後終]之?|[上中下续断开續斷開])(?:部分|章[节節]|段落|[章折回话节幕段阙節話闕])[： 　，、．·。—.,:|-]{0,4}(?:.{0,40}[^。：;；\n]|$))$/
            }, {
                token : ["text", "title1"],
                regex : /^([ 　\s]*)((?:第?[0-9０-９]{1,5}(?:[-—.][0-9０-９]{1,5})?|第?[0０○〇零一二三四五六七八九十百千壹贰叁肆伍陆柒捌玖拾佰仟两廿卅卌貳叄陸兩]{1,7}|最[初终尾終]|[上下]半|[序初前始后终首末尾後終]之?|[上中下续断开續斷開])[卷部集篇册辑季冊輯][： 　，、．·。—.,:|-]{0,4}(?:.{0,40}[^。：;；\n]|$))$/
            }, {
                token : ["text", "title2"],
